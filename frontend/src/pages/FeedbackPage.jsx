...
//  - Requires student login (redirects to /student-login)
...
// Editable emoji faces (user can pick from these)
const EMOJI_FACES = ['😍','🙂','😐','😕','😡','🤩','😋','🤢','😴','🔥']
...
export default function FeedbackPage() {
  ...
  useEffect(() => {
    if (!user) {
      const dest = `/student-login${window.location.search}`
  ...
  async function handleSubmit() {
    ...
    const reqQuestions = questions.filter(q => !['photo_upload','voice_note'].includes(q.question_type || 'single_select'))
          ...
          <div className="card" style={{ textAlign: 'center' }}>
            ...
            {isFlagged && (
              <div style={{ background: '#FFF3E0', border: '2px solid var(--orange)', borderRadius: 10, padding: '14px', marginBottom: 16, fontSize: 13 }}>
                ...
                Giving a photo with your feedback and a genuine comment helps prove authenticity.
              </div>
            )}
            ...
            <div style={{ display: 'flex', gap: 10, justifyContent: 'center' }}>
              <button className="btn btn-primary" onClick={() => {
                ...
                setComment(''); setStaffRating(null); setImageFile(null); setImagePreview('')
              }}>
      ...
      <div style={{ maxWidth: 580, margin: '0 auto' }}>
        ...
        <div style={{ textAlign: 'center', color: 'white', marginBottom: 24 }}>
          ...
          <div style={{ display: 'flex', gap: 8, justifyContent: 'center', marginTop: 14, flexWrap: 'wrap' }}>
            {SLOT_INFO.map(s => (
              <span key={s.label} style={{ background: 'rgba(255,255,255,0.18)', color: 'white', padding: '5px 12px', borderRadius: 99, fontSize: 12 }}>
        ...
        <div className="card">
                  ...
                  <div key={qKey} style={{
                      ...
                      <div>
                        <input type="range" min="1" max="10" step="1"
                          ...
                          onChange={e => {
                            ...
                            const idx  = val - 1
                            recordAnswer(q, val, { emoji: q.emoji_scale?.[idx] || '' })
                          }}
                    ...
                    {qType === 'photo_upload' && (
                      <div style={{ fontSize: 13, color: 'var(--grey)', background: 'white', borderRadius: 8, padding: 10, border: '1px dashed var(--border)' }}>
                        📸 Upload your meal photo below in Step 4 — this helps verify your feedback is real!
                      </div>
                    )}
                    ...
                    {qType === 'voice_note' && (
                      <div style={{ fontSize: 13, color: 'var(--grey)', background: 'white', borderRadius: 8, padding: 10, border: '1px dashed var(--border)' }}>
                        🎙️ Voice notes coming soon! For now, leave a written comment in Step 4.
                      </div>
                    )}
                  </div>
                )
          ...
          {/* Step 3: Staff Behaviour with EDITABLE emoji */}
          <div style={{ marginBottom: 20 }}>
            <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--primary)', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 12 }}>
          ...
          <div style={{ marginBottom: 20 }}>
            ...
            <div className="form-group">
              <label className="label">Upload a Photo <span style={{ color: 'var(--green)', fontSize: 11 }}>⭐ Boosts AI genuineness score!</span></label>
              <div
              ...
              >
                {imagePreview ? (
                  ...
                  <>
                    ...
                    <div style={{ fontSize: 13, color: 'var(--grey)', marginTop: 6 }}>Click to upload or capture photo</div>
                  </>
                )}
              </div>
          ...
          <div style={{
            ...
            🛡️ <strong>FeedGuard AI</strong> checks every submission for authenticity. Random or fake answers reduce your tokens and issue a warning.
          </div>
          ...
          <p style={{ textAlign: 'center', fontSize: 12, color: 'var(--grey)', marginTop: 12 }}>
            🪙 Genuine feedback earns 1–10 tokens • Photos boost your score!
          </p>
        </div>
